      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="p-5 bg-slate-900/60 border border-slate-800/80 rounded-2xl">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider">{t('caregiver.assignedPatients')}</span>
            <Users className="w-5 h-5 text-teal-400" />
          </div>
          <div className="mt-3 text-3xl font-extrabold text-white">{data?.total_assigned_patients ?? 0}</div>
          <div className="mt-1 text-xs text-slate-400">{t('caregiver.assignedPatients')}</div>
        </div>

        <div className="p-5 bg-slate-900/60 border border-slate-800/80 rounded-2xl">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider">{t('schedule.title')}</span>
            <Clock className="w-5 h-5 text-indigo-400" />
          </div>
          <div className="mt-3 text-3xl font-extrabold text-white">{data?.today_doses_scheduled ?? 0}</div>
          <div className="mt-1 text-xs text-slate-400">{t('schedule.subtitle')}</div>
        </div>

        <div className="p-5 bg-slate-900/60 border border-emerald-500/20 rounded-2xl bg-emerald-500/5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-emerald-400 tracking-wider">{t('schedule.markTaken')}</span>
            <CheckCircle2 className="w-5 h-5 text-emerald-400" />
          </div>
          <div className="mt-3 text-3xl font-extrabold text-emerald-300">{data?.today_doses_taken ?? 0}</div>
          <div className="mt-1 text-xs text-emerald-400/80">{t('schedule.markTaken')}</div>
        </div>

        <div className="p-5 bg-slate-900/60 border border-rose-500/20 rounded-2xl bg-rose-500/5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-rose-400 tracking-wider">{t('schedule.missed')}</span>
            <XCircle className="w-5 h-5 text-rose-400" />
          </div>
          <div className="mt-3 text-3xl font-extrabold text-rose-300">{data?.today_doses_missed ?? 0}</div>
          <div className="mt-1 text-xs text-rose-400/80">{t('schedule.missed')}</div>
        </div>
        ...
        <div className="p-5 bg-slate-900/60 border border-slate-800/80 rounded-2xl">
          ...
          <div className="mt-1 text-xs text-slate-400">
            {t('common.status')}: {data?.overall_adherence_status || 'Good'}
          </div>
        </div>
      </div>
