...
import { formatLocalTime, formatLocalDate, formatLocalDateTime } from '../../utils/dateTime';
...
export const formatTakenAtTime = (actualTimeStr) => {
  return formatLocalTime(actualTimeStr, { hour12: true });
};
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5 flex-wrap">
            <h2 className="text-2xl font-bold tracking-tight text-white">
              {t('dashboard.welcomeBack')}, {activeUser?.name || t('auth.patientRole')}
            </h2>
            ...
            {caregiverName && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-300 bg-blue-500/10 px-2.5 py-0.5 rounded-md border border-blue-500/20">
                <span>{t('caregiver.title')}: <strong>{caregiverName}</strong></span>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{t('nav.scanPrescription')}</span>
          </Link>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('caregiver.title')}:</span>
              {caregiverName ? (
                <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-medium text-slate-400">{t('medications.title')}</span>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-medium text-slate-400">{t('schedule.title')}</span>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-medium text-slate-400">{t('conditions.title')}</span>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-medium text-slate-400">{t('prescriptions.title')}</span>
      ...
      {adherence && (
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
            <div>
              <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider">{t('caregiver.adherenceOverview')}</span>
            ...
            <div className="text-xs text-slate-400">
              <span className="text-emerald-400 font-semibold">{adherence.taken_count} {t('schedule.onTime', 'taken')}</span> ·{' '}
              <span className="text-rose-400 font-semibold">{adherence.missed_count} {t('schedule.missed', 'missed')}</span> ·{' '}
              <span className="text-slate-400">{adherence.skipped_count} {t('schedule.markSkipped', 'skipped')}</span>
            </div>
          </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              ...
              {t('schedule.todayDosesTitle', "Today's Scheduled Medication Doses")}
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">{t('schedule.subtitle')}</p>
          </div>
          <button
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center text-slate-400">
            ...
            <p className="text-sm font-medium">{t('dashboard.noDosesToday')}</p>
            <p className="text-xs text-slate-500 mt-1">{t('medications.addFirstMedication')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3">
                  <div className="text-left sm:text-right text-xs">
                    ...
                    {dose.actual_time && (
                      <div className="text-[10px] text-emerald-400">
                        {t('schedule.takenAt')} {formatTakenAtTime(dose.actual_time)}
                      </div>
                    )}
                  </div>
                  ...
                  {dose.status === 'SCHEDULED' ? (
                    <div className="inline-flex items-center gap-2">
                      ...
                      >
                        {actionLoading === dose.id ? t('common.saving') : t('schedule.markTaken')}
                      </button>
                      <button
